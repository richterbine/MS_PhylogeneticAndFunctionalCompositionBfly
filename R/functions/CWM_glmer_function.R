...
cwm.sig.glmm<- function(comm, traits, envir, tree, formula, 
                        AsFactors, PGLS, 
                        runs, random.term){
  ...
  mt<- as.data.frame(cbind(MT, envir[, posit, drop = F]))
  
  for (i in 1:runs){
    MT.null.site <- SYNCSA::permut.row.matrix(mt)$permut.matrix
    data.null.site <- as.data.frame(cbind(MT.null.site, envir[,-posit]))
